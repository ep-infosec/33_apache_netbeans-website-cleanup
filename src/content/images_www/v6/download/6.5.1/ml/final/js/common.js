...
var OMNITURE_CODE_JS = "https://netbeans.org/images_www/js/s_code_remote.js";
...
function startList() {
    // source: https://netbeans.org/branding/scripts/lang-pulldown.js
...
function get_file_name_short(platform, option) {
    ...
    if(platform=="zip") {
        ...
        file_name += (option == "javafx") ? FXBUNDLE_FILES_PREFIX : BUNDLE_FILES_PREFIX;
    }
    if (option != "all") {
    	file_name += "-" + option;
    }
...
function get_file_bouncer_url(platform, option) {
    var url = BOUNCER_URL;
    url += "?" + "product=" + (option == "javafx" ? JAVAFX_BOUNCER_PRODUCT_PREFIX : BOUNCER_PRODUCT_PREFIX);
    if(option != "all") {
        url += "-" + option;
    }
    url += "&" + "os=" + platform;
